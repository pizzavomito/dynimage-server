...
class PackageController {
    ...
    public function packageEditAction(Request $request, Application $app, $packageFile) {
        ...
        $packageContent = file_get_contents($app['dynimage.available_dir'] . '/' . $packageFile);

        $perms = substr(sprintf('%o', fileperms($app['dynimage.available_dir'] . '/' . $packageFile)), -4);

        return $app['twig']->render('Manager\package_edit.html.twig', array(
                    'writable' => is_writable($app['dynimage.available_dir'] . '/' . $packageFile),
    ...
    public function packageDeployReloadAction(Request $request, Application $app) {
        ...
        $app['session']->getFlashBag()->add('success', 'Deploy successfu');
    ...
    public function packageNewAction(Request $request, Application $app) {
        ...
        $referer = $request->headers->get('referer');
        if (file_exists($app['dynimage.available_dir'] . '/' . $newPackage)) {
            $app['session']->getFlashBag()->add('danger', 'New package failed. The file '.$newPackage.' already exist.');
            return $app->redirect($referer);
        }
        if (!is_writable($app['dynimage.available_dir'])) {
        ...
        if (file_put_contents($app['dynimage.available_dir'] . '/' . $newPackage, $tpl) === false) {
    ...
    public function packageDeleteAction(Request $request, Application $app) {
    ...
    public function packageTestAction(Request $request, Application $app, $packageFile) {
        ...
        try {
            file_put_contents($app['dynimage.available_dir'] . '/' . $packageFile, $request->request->get('contents'));
